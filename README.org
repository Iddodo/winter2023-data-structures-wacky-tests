...
1. מקמפלים את הפרויקט שלכם
    src_bash{‏g++ -std=c++11 -Wall -Werror -pedantic-errors -ggdb3 -DNDEBUG ./*.cpp -o main}
2. יוצרים תיקייה לשים בה את התוצאות של הטסטים:
   src_bash{mkdir -p wacky/real-out}
3. בודקים חריגות זיכרון:
   src_bash{‏for i  in {1..100}; do valgrind --leak-check=full  ./main < "wacky/input$i.in" > "wacky/real-out/output$i.out" ; done}
4. משווים את הטסטים לתוצאות שלכם:
   src_bash{‏for i  in {1..100}; do diff -s -q "wacky/out/output$i.out"  "wacky/real-out/output$i.out"  ; done}
